    ...
    public Action<PaymentState, PaymentEvent> preAuthAction() {
        return context -> {
            ...
            if(new Random().nextInt(10) < 8) {
                ...
                sendMessage(context, PaymentEvent.PRE_AUTH_APPROVED);
            } else {
                log.debug("PreAuth declined! No Credit.");
                sendMessage(context, PaymentEvent.PRE_AUTH_DECLINED);
            }
        };
    }
    ...
    public Action<PaymentState, PaymentEvent> authAction() {
        return context -> {
            ...
            if(new Random().nextInt(10) < 8) {
                ...
                sendMessage(context, PaymentEvent.AUTH_APPROVED);
            } else {
                log.debug("Auth declined!");
                sendMessage(context, PaymentEvent.AUTH_DECLINED);
            }
        };
    }

    private void sendMessage(StateContext<PaymentState, PaymentEvent> context, PaymentEvent event) {
